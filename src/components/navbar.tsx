    ...
    >
      <div className="container mx-auto flex h-20 items-center px-4">
        <div className="flex flex-1 items-center justify-between">
          ...
          {/* Desktop Navbar */}
          <nav className="hidden lg:flex items-center lg:space-x-6 space-x-4">
            <div className="h-10 bg-gray-800 px-4 flex items-center rounded-full gap-4 space-x-4 mx-4">
              {NAV_ITEMS.map((item) => (
                <motion.button
                  key={item.id}
                  whileHover={{ scale: 1.05 }}
                  whileTap={{ scale: 0.95 }}
                  className="text-secondary-foreground text-sm md:text-md hover:text-foreground transition-colors duration-200 font-semibold cursor-pointer"
